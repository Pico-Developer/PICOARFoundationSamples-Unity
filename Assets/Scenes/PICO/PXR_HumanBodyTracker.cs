...
{
    public class PXR_HumanBodyTracker : MonoBehaviour
    {
        [SerializeField]
        ...
        Dictionary<TrackableId, PXR_BoneController> m_SkeletonTracker = new Dictionary<TrackableId, PXR_BoneController>();
        ...
        {
            PXR_BoneController boneController;
            ...
            foreach (var humanBody in eventArgs.added)
            {
                if (!m_SkeletonTracker.TryGetValue(humanBody.trackableId, out boneController))
                {
                    ...
                    if (!GameObject.Find(m_SkeletonPrefab.name))
                    {
                        Debug.LogFormat("OnHumanBodiesChanged m_SkeletonPrefab no ");
                        var newSkeletonGO = Instantiate(m_SkeletonPrefab, humanBody.transform);
                        boneController = newSkeletonGO.GetComponent<PXR_BoneController>();
                    }
                    else
                    {
                        Debug.LogFormat("OnHumanBodiesChanged m_SkeletonPrefab has ");
                        boneController = m_SkeletonPrefab.GetComponent<PXR_BoneController>();
                    }
